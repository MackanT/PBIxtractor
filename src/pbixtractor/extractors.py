...
from .models import ExtractedItem, ExtractedFilter
...
class BaseExtractor(ABC):
    ...
    def __init__(self, config: dict, log_callback: callable = None):
        ...
        Args:
            ...
            log_callback: Function to call for logging (severity, message, data)
        """
        self.config = config
        self.log = log_callback or self._default_log
    
    def _default_log(self, message: str, data: Any = None, severity: int = 0):
        """Default logging (print to console)."""
        severity_map = {-1: "DEBUG", 0: "INFO", 1: "WARNING", 2: "ERROR", 3: "CRITICAL"}
        print(f"[{severity_map.get(severity, 'INFO')}] {message}")
        if data:
...
class VisualExtractor(BaseExtractor):
    ...
    def __init__(self, config: dict, visual_types: list, data_types: list, log_callback: callable = None):
        """
    ...
    def extract(self, visual_container: dict, page_name: str) -> list[ExtractedItem]:
        ...
        # Unknown visual type
        self.log(f"Unknown visual type: {visual_type}", visual_container, 1)
        return items
    
    def _extract_standard_visual(self, config: dict, page_name: str, item_name: str, visual_type: str) -> list[ExtractedItem]:
    ...
    def _extract_table_and_field(self, row: dict) -> tuple[str, str]:
        ...
        self.log("Could not extract table/field", row, 1)
        return "", ""
...
class FilterExtractor:
    ...
    def extract_visual_filters(self, filters: list, item_name: str, page_name: str) -> list[ExtractedFilter]:
        ...
        for filter_obj in filters:
            ...
            extracted.append(ExtractedFilter(
                page=page_name,
                item_name=item_name,
                filter_type="Visual",
    ...
    def extract_page_filters(self, filters: list, page_name: str) -> list[ExtractedFilter]:
        ...
        for filter_obj in filters:
            ...
            if filter_variant == "Categorical":
                ...
                self.log(f"Unknown filter variant: {filter_variant}", filter_obj, 1)
                continue
    ...
    def _parse_categorical_filter(self, filter_obj: dict) -> tuple[str, str]:
        ...
        is_inverted = self.query_json(filter_obj, "$..isInvertedSelectionMode.expr.Literal.Value", False)
...
class PageExtractor(BaseExtractor):
    ...
    def __init__(self, config: dict, visual_types: list, data_types: list, log_callback: callable = None):
        ...
        super().__init__(config, log_callback)
        self.visual_extractor = VisualExtractor(config, visual_types, data_types, log_callback)
        self.filter_extractor = FilterExtractor(config, log_callback)
    ...
    def extract(self, page: dict) -> tuple[list[ExtractedItem], list[ExtractedFilter]]:
        ...
        try:
            ...
            self.log("Failed to parse page filters", page_filters_str, 2)
